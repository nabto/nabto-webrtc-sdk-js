...
export class JWTMessageSigner implements MessageSigner {
  ...
  async signMessage(msg: JSONValue): Promise<ProtocolSigningMessage> {
    ...
    const signed = rs.KJUR.jws.JWS.sign("HS256", JSON.stringify(header), JSON.stringify(payload), { "rstr": this.sharedSecret });
  ...
  async verifyMessage(message: JSONValue): Promise<JSONValue> {
    ...
    const verified = rs.KJUR.jws.JWS.verifyJWT(token, { "rstr": this.sharedSecret }, { alg: ["HS256"] })
    if (!verified) {
